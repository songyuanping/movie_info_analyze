...
class CommentSpider:
    ...
    def __init__(self):
        ...
        #使用cookies爬取评论数据
        self.cookies={'cookie':'bid=UvGRE8zcFmY; _vwo_uuid_v2=DAD471B0999F79F0DB1EFBD8994B810F5|f1b0fd889e05906c8f40b0080ce1e828; __gads=ID=442879fd71c39f36:T=1586481585:S=ALNI_MatzEFy5JKEujx31fh7seqiYy3fbA; push_noty_num=0; push_doumail_num=0; __utmv=30149280.21336; ll="128099"; douban-fav-remind=1; __utmc=30149280; ct=y; __utmz=30149280.1591336871.21.18.utmcsr=cn.bing.com|utmccn=(referral)|utmcmd=referral|utmcct=/; dbcl2="213360799:EHt0YW1xFuE"; ck=PRMJ; frodotk="56cf928987ae1c7f046eb79c970a557d"; __utma=30149280.474729895.1586308492.1591336871.1591339221.22; __utmb=30149280.0.10.1591339221'}
...
class MovieSpider:
    ...
    def __init__(self):
        ...
        self.header= {'User-Agent': UserAgent().random,
                     'cookie':'bid=UvGRE8zcFmY; _vwo_uuid_v2=DAD471B0999F79F0DB1EFBD8994B810F5|f1b0fd889e05906c8f40b0080ce1e828; __gads=ID=442879fd71c39f36:T=1586481585:S=ALNI_MatzEFy5JKEujx31fh7seqiYy3fbA; push_noty_num=0; push_doumail_num=0; __utmv=30149280.21336; ll="128099"; douban-fav-remind=1; __utmc=30149280; ct=y; __utmz=30149280.1591336871.21.18.utmcsr=cn.bing.com|utmccn=(referral)|utmcmd=referral|utmcct=/; dbcl2="213360799:EHt0YW1xFuE"; ck=PRMJ; frodotk="56cf928987ae1c7f046eb79c970a557d"; __utma=30149280.474729895.1586308492.1591336871.1591339221.22; __utmb=30149280.0.10.1591339221'
                     }
        #电影信息列表
        self.movieList = []
        #电影的详情url和电影名称
    ...
    def get_movie_info(self):
        ...
        while self.start <= 225:
            ...
            for movie in movies:
                ...
                response=requests.get(str(movie[1]),headers=self.header).text
    ...
    with open(r'film.json','r',encoding='utf-8') as file:
      ...
      for item in data:
         ...
         if index>=110:
             ...
             print(item[0])
             print(item[1])
             commentSpider.download(item[0],item[1],35)
         index+=1
    '''
